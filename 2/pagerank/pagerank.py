...
def sample_pagerank(corpus, damping_factor, n):
    ...
    for i in range(1, n):
        # important function
        result = random.choices(population=list(now_trans.keys()), 
                                weights=list(now_trans.values()))[0]
        freq[result] += 1
    ...
    while True:
        ...
        for page in corpus.keys():
            freq[page] = (1-damping_factor)/l + \
                damping_factor * sum([freqcopy[father] / len(corpus[father])
                                     for father in father_page[page]])
        if kill(freqcopy, freq):
            break
    return freq
